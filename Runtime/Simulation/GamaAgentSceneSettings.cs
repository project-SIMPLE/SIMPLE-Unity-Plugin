...
public class GamaAgentSceneSettings : MonoBehaviour
{
    [Header("Middleware import")]
    [SerializeField] private bool createAgentEntries = true;
    [SerializeField] private int maxAgentEntries = 500;
    ...
    private readonly Dictionary<string, GamaAgentPropertySettings> propertyLookup =
    ...
    public void ImportProperties(IEnumerable<PropertiesGAMA> properties, int precision)
    {
        RebuildLookups();
        ...
        foreach (PropertiesGAMA property in properties)
        {
            ...
            if (!propertyLookup.TryGetValue(property.id, out settings))
            {
                settings = new GamaAgentPropertySettings();
                propertySettings.Add(settings);
                propertyLookup[property.id] = settings;
            }
        ...
        int precision)
    {
        ...
        if (property != null &&
            ...
            propertyLookup.TryGetValue(property.id, out propertyOverride))
        {
        ...
        if (!agentLookup.TryGetValue(agentName, out settings))
        {
            if (maxAgentEntries > 0 && agentSettings.Count >= maxAgentEntries)
            {
                if (logWhenAgentEntriesCapReached && !maxEntriesWarningLogged)
                {
                    ...
                    Debug.LogWarning(
                        "[GAMA] GamaAgentSceneSettings reached maxAgentEntries=" +
                        maxAgentEntries +
    ...
    private void RebuildLookups()
    {
        ...
        for (int i = 0; i < propertySettings.Count; i++)
        {
            ...
            if (settings != null && !string.IsNullOrEmpty(settings.PropertyId) && !propertyLookup.ContainsKey(settings.PropertyId))
            {
                propertyLookup.Add(settings.PropertyId, settings);
            }
        }
