...
public class GamaPrefabSceneSettings : MonoBehaviour
{
    [Header("Imported GAMA prefab references")]
    [SerializeField] private bool createPropertyEntries = true;
    [SerializeField] private bool keepManualPropertyEntriesWhenMissing = true;
    ...
    private readonly Dictionary<string, GamaPrefabPropertyBinding> propertyLookup =
    ...
    public void ImportProperties(IEnumerable<PropertiesGAMA> properties)
    {
        RebuildLookups();
        ...
        foreach (PropertiesGAMA property in properties)
        {
            ...
            bool hasEntry = propertyLookup.TryGetValue(property.id, out binding);
            ...
            if (!hasEntry)
            {
                ...
                propertyLookup[property.id] = binding;
            }

            binding.ImportFrom(property);
        }

        if (!keepManualPropertyEntriesWhenMissing)
        {
        ...
        out string signature)
    {
        ...
        if (!string.IsNullOrWhiteSpace(property.id) &&
            propertyLookup.TryGetValue(property.id, out propertyBinding))
        {
    ...
    private void LogMissingPrefab(PropertiesGAMA property, List<string> candidateKeys)
    {
        ...
        Debug.LogWarning(
            ...
            "'). Tried keys: " +
            candidates +
            ". Configure GamaPrefabSceneSettings mappings on SimulationManager.");
    }
    ...
    private void RebuildLookups()
    {
        ...
        for (int i = 0; i < propertyBindings.Count; i++)
        {
            ...
            if (!propertyLookup.ContainsKey(binding.PropertyId))
            {
                propertyLookup.Add(binding.PropertyId, binding);
            }
        }
    }
}
...
public class GamaPrefabPropertyBinding
{
    ...
    public void ImportFrom(PropertiesGAMA property)
    {
        ...
        importedPrefabNormalized = GamaPrefabSceneSettings.NormalizeKey(property.prefab);
    }

    public bool TryResolveManualPrefab(
        GamaPrefabSceneSettings settings,
        out GameObject prefab,
        out string signature)
    {
        prefab = null;
...
public class GamaPrefabKeyTranslation
{
    ...
    public bool Matches(string candidate)
    {
        ...
        string expected = GamaPrefabSceneSettings.NormalizeKey(gamaKey);
        string current = GamaPrefabSceneSettings.NormalizeKey(candidate);
    ...
    public bool TryResolveManualPrefab(GamaPrefabSceneSettings settings, out GameObject prefab)
    {
        prefab = null;
        if (!enabled)
        {
